...
public class HeapSortObitos {
	...
	private static void maxHeapifica(int pai) {
		...
		if (esquerda <= tam && dataSet.getDados()[esquerda].getDeaths() > dataSet.getDados()[maior].getDeaths())
			maior = esquerda;

		if (direita <= tam && dataSet.getDados()[direita].getDeaths() > dataSet.getDados()[maior].getDeaths())
			maior = direita;
